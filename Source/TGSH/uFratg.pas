...
uses
  ...
  uFrmFJ, DateUtils, uDDCX_BZ, uPub_Text,uTGCYDlgFrm;
...
var
  SelItem: TListItem;
  iOrderID,iType,n: integer;
  sSqlData, sCpbh, str: string;
  ADOQ_Rec,ADOQ_Insert: TADOQuery;
  sYL: string;
begin
  SelItem := lv_jhxd.Selected;
  if not Assigned(SelItem) then Exit;
  ...
  begin
    try
      ...
      sSqlData := 'select F_iID,F_iOrderID,F_tiOrderType,F_sCPBH,%s YL,F_sYztmc,F_sTSGY,F_iXGR_TSGY,F_iCZR,F_dCXRQ,F_tiNJBZ '
        + 'from DO_OrderApart where F_tiOrderType=%d and F_iOrderID=%d and F_tiCXBZ=0';
      ADOQ_Rec := DM_DataBase.OpenQuery(sSqlData,[sYL,iType,iOrderID],True);
      ...
      begin
        ...
        begin
          SetLength(TGCYDlgFrm.aCYDDInfo,n+1);
          TGCYDlgFrm.aCYDDInfo[n].m_iAPartID := ADOQ_Rec.FieldByName('F_iID').AsInteger;
          TGCYDlgFrm.aCYDDInfo[n].m_iOrderID := ADOQ_Rec.FieldByName('F_iOrderID').AsInteger;
          TGCYDlgFrm.aCYDDInfo[n].m_tiOrderType := ADOQ_Rec.FieldByName('F_tiOrderType').AsInteger;
          TGCYDlgFrm.aCYDDInfo[n].m_sCPBH := ADOQ_Rec.FieldByName('F_sCPBH').AsString;
          TGCYDlgFrm.aCYDDInfo[n].m_nYL := ADOQ_Rec.FieldByName('YL').AsFloat;
          TGCYDlgFrm.aCYDDInfo[n].m_sYztmc := ADOQ_Rec.FieldByName('F_sYztmc').AsString;
          TGCYDlgFrm.aCYDDInfo[n].m_iFlag := 0;
          Inc(n);
          ADOQ_Rec.Next;
        end;
      end;
      if TGCYDlgFrm.ShowModal = IDOK then
      begin
        try
          DM_DataBase.Con_YDPrint.BeginTrans;
          ADOQ_Rec.First;
          for n := 0 to Length(TGCYDlgFrm.aCYDDInfo) -1 do
          begin
            with TGCYDlgFrm.aCYDDInfo[n] do
            begin
              if m_iFlag = 0 then
              begin
                sSqlData := 'Select * from DO_OrderApart where 1=2';
                ADOQ_Insert := DM_DataBase.OpenQuery(sSqlData,[],True);

                while not ADOQ_Rec.Eof do
                begin
                  if ADOQ_Rec.FieldByName('F_iID').AsInteger = m_iAPartID then
                  begin
                    ADOQ_Insert.Insert;
                    ADOQ_Insert.FieldByName('F_iOrderID').AsInteger := ADOQ_Rec.FieldByName('F_iOrderID').AsInteger;
                    ADOQ_Insert.FieldByName('F_tiOrderType').AsInteger := ADOQ_Rec.FieldByName('F_tiOrderType').AsInteger;
                    ADOQ_Insert.FieldByName('F_sCPBH').AsString := ADOQ_Rec.FieldByName('F_sCPBH').AsString;
                    ADOQ_Insert.FieldByName('F_nYL').AsFloat := m_nYL;
                    ADOQ_Insert.FieldByName('F_sYztmc').AsString := ADOQ_Rec.FieldByName('F_sYztmc').AsString;
                    ADOQ_Insert.FieldByName('F_sTSGY').AsString := ADOQ_Rec.FieldByName('F_sTSGY').AsString;
                    ADOQ_Insert.FieldByName('F_iXGR_TSGY').AsVariant := ADOQ_Rec.FieldByName('F_iXGR_TSGY').AsVariant;
                    ADOQ_Insert.FieldByName('F_iCZR').AsVariant := ADOQ_Rec.FieldByName('F_iCZR').AsVariant;
                    ADOQ_Insert.FieldByName('F_dCXRQ').AsVariant := ADOQ_Rec.FieldByName('F_dCXRQ').AsVariant;
                    ADOQ_Insert.FieldByName('F_tiNJBZ').AsVariant := ADOQ_Rec.FieldByName('F_tiNJBZ').AsVariant;
                    ADOQ_Insert.FieldByName('F_tiCXBZ').AsInteger := 0;
                    ADOQ_Insert.FieldByName('F_tiCYBZ').AsInteger := 1;
                    ADOQ_Insert.Post;
                    Break;
                  end;
